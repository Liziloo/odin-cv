    ...
    return (
      <section>
        <form onSubmit={handleSubmit}>
          ...
          <CustomInput id="company-responsibilities" label="Responsibilities:" text={responsibilities} />
        ...
        {experience.length > 0 ? 
          experience.map((ex) => {
            return (
              <div key={ex.id} className="experience">
                <h2>{ex.company}</h2>
                <h3>{ex.position}</h3>
                <h4>{ex.years}</h4>
        ...
        {experience.map((ex) => {
            return (
              <div key={ex.id} className="experience">
                ...
                <h4>{ex.years}</h4>
                <p>{ex.responsibilities}</p>
                <button data-experience-id={ex.id} onClick={handleEditButton}>
                  Edit
                </button>
                <button  data-experience-id={ex.id} onClick={handleSchoolDelete}>
                  Delete
                </button>
              </div>
            );
